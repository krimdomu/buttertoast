...
sub dispatch_query {
    ...
    my @service_keys = grep { $self->buttertoast->redis_rw->get($_) eq $service_name } $self->buttertoast->redis_rw->keys("service:*:name");
    ...
    if (@service_keys) {
        ...
        my $count = $self->buttertoast->redis_rw->get($count_key);

        if($count && $count > 0) {
            if ( $qtype eq "A" && $qname ) {
                for my $i (0..$count-1) {
                    my $ip = $self->buttertoast->redis_rw->get("$key_base:$i:ip");
